  ...
  return (
    <article className="max-w-7xl mx-auto px-6 md:px-12 xl:px-6">
      <div class=" grid divide-x divide-y divide-gray-100 dark:divide-gray-700 overflow-hidden rounded-3xl border border-gray-100 text-gray-600 dark:border-gray-700 sm:grid-cols-1 lg:grid-cols-3 lg:divide-y-0 xl:grid-cols-3 flex-wrap card">
        <img
          class="object-cover image w-full min-w-[400px]"
          src={image}
          alt={`image about ${image}`}
        />

        <div class="p-5 bg-white relative w-1/2 h-auto min-w-[400px]">
        ...
        <video
          className="aspect-square min-w-[400px]"
          src={video1}
          autoPlay
          loop
          muted
        ...
        <a
          onClick={handleBack}
          class="absolute top-2 right-2 inline-flex px-4 py-2 bg-gradient-to-r from-[#030f33] to-[#4e737a] rounded-full hover:scale-105 cursor-pointer btn-back"
        >
          Volver
        </a>
      </div>
